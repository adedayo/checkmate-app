...
// something truthful. A test that only asserted "dev" would pass for a version
// package with no fallback at all, so this asserts the weaker but meaningful
// property: never empty.
...
func TestInfoString(t *testing.T) {
	cases := []struct {
		...
		{
			name: "version only",
			info: Info{Version: "v2.2.0"},
			want: "v2.2.0",
		},
		{
			name: "version and commit",
			info: Info{Version: "v2.2.0", Commit: "abc123"},
			want: "v2.2.0 (abc123)",
		},
		{
			name: "fully stamped",
			info: Info{Version: "v2.2.0", Commit: "abc123", BuildDate: "2026-01-01T00:00:00Z"},
			want: "v2.2.0 (abc123) built 2026-01-01T00:00:00Z",
		},
	}
...
func TestInfoLongIncludesEveryPopulatedField(t *testing.T) {
	...
	got := info.Long("checkmate-app")

	for _, want := range []string{"checkmate-app", "v2.2.0", "abc123", "2026-01-01T00:00:00Z", "go1.26.1", "darwin/arm64"} {
...
func TestInfoLongOmitsEmptyFields(t *testing.T) {
	got := Info{Version: "dev", GoVersion: "go1.26.1", Platform: "linux/amd64"}.Long("checkmate-app")
...
// The whole reason LDFlags exists is that the import path is easy to mistype
// and a typo produces a binary that reports "dev" with no build error.
// Asserting the exact path is therefore the assertion that matters.
func TestLDFlagsStampsTheCorrectImportPath(t *testing.T) {
	got := LDFlags("v2.2.0", "abc123", "2026-01-01T00:00:00Z")

	for _, want := range []string{
		"-X checkmate-app/pkg/version.Version=v2.2.0",
		"-X checkmate-app/pkg/version.Commit=abc123",
		"-X checkmate-app/pkg/version.BuildDate=2026-01-01T00:00:00Z",
	} {
		if !strings.Contains(got, want) {
